...
%options = optimset('Algorithm','trust-region-reflective' ,'GradObj','on', 'Hessian','on', 'TolFun', 1e-7,'Display','off');
...
% [x, fval, exitflag, output] = fmincon(@(x) fobj_basis_dual(x, SSt, XSt, X, c, trXXt), dual_lambda, [], [], [], [], [], [], [], options);
% output.iterations
fval_opt = -0.5*N*fval;
dual_lambda= x;
