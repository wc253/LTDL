...
dataPath = ['E:\gong\CVPR_model\tsp_demo_code\ex_MSI_denoising_2019imshow\detection\data\Jasper Ridge\end4']; 
load(dataPath);
dataPath = ['E:\gong\CVPR_model\tsp_demo_code\ex_MSI_denoising_2019imshow\detection\data\Jasper Ridge\jasperRidge2_R198']; 
load(dataPath);

b_s = 31;
b_e = b_s + 39;
select_bands = [b_s:b_e];
target = M(select_bands, 4);
h=100;w=100;p=40;
M=Y(select_bands, :)/maxValue;
SNR = 3000;
NOISE=randn(size(M));
signal_power = 1/length(M(:))*sum(M(:).*M(:));
noise_variance = signal_power / ( 10^(SNR/10) );
nu = sqrt(noise_variance);
M = M+nu*NOISE;
noisy_msi = reshape(M', [nRow, nCol, length(select_bands)]);
