function [net,stats] = cnn_train_dag_weightedL2(net, imdb, getBatch, varargin)
...
for t=1:params.batchSize:numel(subset)
    ...
    if strcmp(mode, 'train')
        % forward G
        netG.mode = 'normal' ;
%         netG.forward({'scaled_1_input',noise{1},'label',data,'mask',repmat(mask,1,1,3,1)});
        netG.forward({'scaled_1_input',noise{1},'label',data,'mask',mask});
        label_est =  netG.vars(netG.getVarIndex('Tanh')).value;
        label_est(~mask) = data(~mask);                                      % 将非mosaic区域赋值为清晰图像；
