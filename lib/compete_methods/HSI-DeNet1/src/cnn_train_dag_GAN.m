function [net,stats] = cnn_train_dag_GAN(net, imdb, getBatch, varargin)
...
opts.derOutputsG = {'Loss', 1} ;
opts.derOutputsD = {'logistic', 1};
...
for t=1:params.batchSize:numel(subset)
    ...
    inputs = params.imdb.inputs(:,:,:,batch);
    labels = params.imdb.labels(:,:,:,batch);
    label0 = params.imdb.images.label0(:,:,:, batch);
    label1 = params.imdb.images.label1(:,:,:, batch);
    ...
    if strcmp(mode, 'train')
       ...
       %% forward G
        ...
        netG.mode = 'normal' ;
        netG.forward({'input',noise,'data',data});
        label_est =  netG.vars(netG.getVarIndex('conv16')).value;

       %% update D
        netD.mode = 'normal' ;
        netD.forward({'data_D', label_est, 'label', label0});
        netD.backward( params.derOutputsD);
        stateD = accumulateGradients(netD, stateD, params, batchSize, parserv) ;

        netD.forward({'data_D', data, 'label', label1});
        netD.backward( params.derOutputsD);
        stateD = accumulateGradients(netD, stateD, params, batchSize, parserv) ;
        ...
        netD.mode = 'normal';
        label1 = label1.*0+1;
        netD.forward({'data_D',label_est, 'label', label1});
        netD.backward( params.derOutputsD); 
        Recon_der = vllab_nn_L2_loss(label_est, data, 1);
        netG.backward({'conv16', netD.vars(1).der + Recon_der});
%         netG.backward({params.derOutputsG{1}, netD.vars(1).der + Recon_der});
        ...
        netD.mode = 'test' ;
        netD.eval({'data_D',cat(4, data, label_est) 'label', cat(4, label1, label0)});
    end
...
sel =[ sel, find(cellfun(@(x) isa(x,'dagnn.vllab_dag_loss'), {net.layers.block}))] ;%bae
for i = 1:numel(sel)
