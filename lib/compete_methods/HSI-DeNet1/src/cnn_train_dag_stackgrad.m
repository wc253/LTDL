function [net,stats] = cnn_train_dag_stackgrad(net, imdb, getBatch, varargin)
...
for t=1:params.batchSize:numel(subset)
    ...
    label1   = inputs{8}; 
    mask     = inputs{10};
    noise{1} = cat(3,noise{1},mask);
    
    if strcmp(mode, 'train')
        % forward G
        netG.mode = 'normal' ;
        netG.forward({'scaled_1_input',noise{1}});
        label_est =  netG.vars(netG.getVarIndex('Tanh')).value;
        % update D
        netD.mode = 'normal' ;
        ...
        Construction_grad = wL2_grad(data, label_est, repmat(mask,1,1,3,1));
        stack_Grad = netD.vars(1).der + params.lambda * Construction_grad;
        netG.backward({params.derOutputsG{1}, stack_Grad});
