function [net,stats] = cnn_train_dag_ms(net, imdb, getBatch, varargin)
...
%     [inputs,labels] = params.getBatch(params.imdb, batch) ;
...
%     data   = labels*2-1;
    ...
    if strcmp(mode, 'train')
       % forward G
        ...
        netG.mode = 'normal' ;
%         netG.eval({'input',noise,'data',data}, params.derOutputsG, 'holdOn', t < params.numSubBatches) ;
        netG.forward({'input',noise,'data',data});
        netG.backward({params.derOutputsG{1},params.derOutputsG{2}});
        stateG = accumulateGradients(netG, stateG, params, batchSize, parserv) ;       

%         if t == 1 %bae
%             label_est = mean(label_est,3);
%             im_out = uint8((gather(label_est*256)));        
%             im_out = uint8(get_tensor2img(im_out));
...
%         end
        ...
        netG.mode = 'test' ;
        netG.forward({'input',noise,'data',data});
%         label_est =  netG.vars(netG.getVarIndex('x48')).value;
    end
...
% -------------------------------------------------------------------------
sel = find(cellfun(@(x) isa(x,'dagnn.vllab_dag_loss'), {net.layers.block})) ;
